...
const InteriorBuildPage = () => {
  ...
  let interiorBuildProjects = projects.filter(project => 
    interiorBuildCategories.includes(project.category)
  )

  // Apply additional filters
  if (selectedYear) {
    interiorBuildProjects = interiorBuildProjects.filter(project => project.year === selectedYear)
  }
  if (selectedLocation) {
    interiorBuildProjects = interiorBuildProjects.filter(project => project.location === selectedLocation)
  }
  if (selectedCategory) {
    interiorBuildProjects = interiorBuildProjects.filter(project => project.category === selectedCategory)
  }

  // Get unique values for filter options
  const availableYears = [...new Set(projects.filter(p => interiorBuildCategories.includes(p.category)).map(p => p.year))].sort()
  const availableLocations = [...new Set(projects.filter(p => interiorBuildCategories.includes(p.category)).map(p => p.location))].sort()
  const availableCategories = [...new Set(projects.filter(p => interiorBuildCategories.includes(p.category)).map(p => p.category))].sort()

  const totalSlides = Math.ceil(interiorBuildProjects.length / 2)
  ...
  const getAllProjectPairs = () => {
    ...
    for (let i = 0; i < totalSlides; i++) {
      ...
      const pair = interiorBuildProjects.slice(startIndex, startIndex + 2)
      pairs.push(pair)
    }
    return pairs
  }
