...
const RenovationPage = () => {
  ...
  let renovationProjects = projects.filter(project => 
    renovationCategories.includes(project.category)
  )

  // Apply additional filters
  if (selectedYear) {
    renovationProjects = renovationProjects.filter(project => project.year === selectedYear)
  }
  if (selectedLocation) {
    renovationProjects = renovationProjects.filter(project => project.location === selectedLocation)
  }
  if (selectedCategory) {
    renovationProjects = renovationProjects.filter(project => project.category === selectedCategory)
  }

  // Get unique values for filter options
  const availableYears = [...new Set(projects.filter(p => renovationCategories.includes(p.category)).map(p => p.year))].sort()
  const availableLocations = [...new Set(projects.filter(p => renovationCategories.includes(p.category)).map(p => p.location))].sort()
  const availableCategories = [...new Set(projects.filter(p => renovationCategories.includes(p.category)).map(p => p.category))].sort()

  const totalSlides = Math.ceil(renovationProjects.length / 2)
  ...
  const getAllProjectPairs = () => {
    ...
    for (let i = 0; i < totalSlides; i++) {
      ...
      const pair = renovationProjects.slice(startIndex, startIndex + 2)
      pairs.push(pair)
    }
    return pairs
  }
